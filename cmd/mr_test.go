...
func Test_mrCmd_Milestone(t *testing.T) {
	...
	t.Run("create", func(t *testing.T) {
		...
		out := string(b)
		t.Log(out)
		require.Contains(t, out, "http://localhost:8080/root/test/-/merge_requests")

		i := strings.Index(out, "/diffs\n")
		mrID = strings.TrimPrefix(out[:i], "http://localhost:8080/root/test/-/merge_requests/")
		t.Log(mrID)
	})
	t.Run("list", func(t *testing.T) {
		if mrID == "" {
...
func Test_mrCmd_assign_and_review(t *testing.T) {
	...
	mrURL := "http://localhost:8080/root/test/-/merge_requests/" + mrID
